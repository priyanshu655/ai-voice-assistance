  ...
  <script src="${CLIENT_URL}/assistant.js" data-user-id="${user?._id}"></script>
              ...
              <div className='relative'>
               <textarea readOnly value={embedCode} className='w-full h-20 bg-[#0b1020] text-emerald-400 rounded-2xl p-4 text-sm font-mono resize-none outline-none'/>
                ...
                }} className='absolute top-4 right-4 w-10 h-10 rounded-xl bg-white flex items-center justify-center cursor-pointer'><FiCopy/></button>
              </div>

              <button onClick={()=>setEditAssistant(true)} className='mt-6 h-12 px-6 rounded-2xl bg-gradient-to-r from-purple-500 to-emerald-500 text-white font-medium cursor-pointer'>Edit Assistant</button>
          </div>
        )}
        ...
        {editAssistant && <div className='space-y-6'>
          <div className='bg-white rounded-3xl border border-gray-100 shadow-sm p-6'>
            ...
            <div className='space-y-4'>
              ...
              className="w-full border border-gray-200 rounded-2xl px-4 py-3 outline-none focus:border-purple-500 transition-colors" />
              ...
              className="w-full border border-gray-200 rounded-2xl px-4 py-3 resize-none outline-none focus:border-purple-500 transition-colors" />
            </div>
          </div>
              ...
              <div className='grid grid-cols-2 sm:grid-cols-4 gap-3'>
                {THEMES.map((item)=>(
                  ...
                  onClick={()=>setTheme(item)}
                   className={`py-3 rounded-2xl border-2 capitalize transition-all cursor-pointer ${theme === item
              ...
              <div className='grid grid-cols-2 sm:grid-cols-3 gap-3'>
                {TONES.map((item)=>(
                  ...
                  onClick={()=>setTone(item)}
                   className={`py-3 rounded-2xl border-2 capitalize transition-all cursor-pointer ${tone === item
                        ? "border-purple-500 bg-purple-50 text-purple-700"
                        : "border-gray-200 hover:border-purple-300"
                        }`}>{item}
                  </button>
          ...
          <div className='bg-white rounded-3xl border border-gray-100 shadow-sm p-6'>
            <div className='flex items-center justify-between mb-5 flex-wrap gap-4'>
              ...
              <button onClick={addPage} className='flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-purple-500 to-emerald-500 text-white text-sm cursor-pointer hover:scale-[1.02] transition-transform'>
                <FiPlus/>Add
              </button>
            </div>

            <div className='grid grid-cols-1 sm:grid-cols-3 gap-3'>
              <input type="text" placeholder='Page Name' className='border border-gray-200 rounded-2xl px-4 py-3 outline-none focus:border-purple-500 transition-colors'
              onChange={(e)=>setPageName(e.target.value)}
              value={pageName}/>

              <input type="text" placeholder='/pricing' className='border border-gray-200 rounded-2xl px-4 py-3 outline-none focus:border-purple-500 transition-colors'
              onChange={(e)=>setPagePath(e.target.value)}
              value={pagePath}/>

              <input type="text" placeholder='Pricing Plan' className='border border-gray-200 rounded-2xl px-4 py-3 outline-none focus:border-purple-500 transition-colors'
            ...
            <div className='mt-5 space-y-3'>
              {
                pages.map((page,index)=>(
                  ...
                  className='flex items-center justify-between border border-gray-100 rounded-2xl p-4'>
                    ...
                    <button onClick={()=>removePage(index)} className='text-red-500 hover:bg-red-50 p-2 rounded-lg transition-colors cursor-pointer'>
                      <FiTrash2/>
                    </button>
                  </div>
                ))
        ...
        !businessType ||
        !businessDescription ||
        !geminiApiKey} className='w-full h-14 rounded-2xl bg-gradient-to-r from-purple-500 to-emerald-500 text-white font-semibold disabled:opacity-50 disabled:cursor-not-allowed hover:scale-[1.01] transition-all cursor-pointer'>
            {
